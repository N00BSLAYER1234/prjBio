...
async function renderCartItems() {
  ...
  cart.forEach((item, index) => {
    ...
    const description = productData.description || 'Descrizione non disponibile';
    ...
    div.innerHTML = `
      ...
      <div class="flex-1">
        <p class="font-bold mb-2 text-lg text-gray-800">${item.product}</p>
        <p class="text-sm text-gray-600 mb-2 italic">${description}</p>
        ${unit ? `<p class="text-xs text-gray-500 mb-1">Formato: ${unit}</p>` : ''}
        ${origin ? `<p class="text-xs text-gray-500 mb-2">Origine: ${origin}</p>` : ''}
        <div class="flex gap-4 text-sm mb-1">
          <span class="text-gray-700">Prezzo: €${item.price.toFixed(2)}</span>
          <span class="text-gray-700">Quantità: <strong>${item.quantity}</strong></span>
        </div>
        <p class="text-sm font-semibold text-green-600">Subtotale: €${itemTotal.toFixed(2)}</p>
      </div>
      <div class="flex flex-col gap-1">
        <button onclick="incrementItem(${index})" class="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition">+</button>
        <button onclick="decrementItem(${index})" class="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition">-</button>
        <button onclick="removeItem(${index})" class="px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition">Rimuovi</button>
      </div>
    `;
